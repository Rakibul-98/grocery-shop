...
const Favorites = () => {
  ...
  const removeFavItem = (_id) => {
    const remaining = savedProducts.filter((product) => product._id !== _id);
    setSavedProducts(remaining);
    removeFromDb(_id, "fav");
  };
      ...
      <div className="modal justify-end">
        <div className="modal-box h-screen w-96 mr-5">
          ...
          {savedProducts.length ? (
                  ...
                  {savedProducts.map((product) => (
                    <tr key={product._id}>
                      ...
                      <td className="text-red-600">
                        <button
                          className="p-2 rounded-full bg-red-100 hover:text-orange-400"
                          onClick={() => removeFavItem(product._id)}
                        >
                          <FaTrashAlt />
                        </button>
                      </td>
            ...
            <p className="text-xl text-orange-500 text-center mt-16">
              You didn't add any item yet. <br />
